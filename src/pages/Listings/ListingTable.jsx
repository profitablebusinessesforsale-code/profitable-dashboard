...
export default function ListingTable({ businessRole = "", status = "", searchTerm }) {
  ...
  const dataSource = listingsData?.data?.map((listing, index) => ({
    ...
    interestCount: listing?.totalInterest || 0,
    ...
    },
    {
      ...
      render: (_, record) => (
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col items-start justify-center">
            <span>
              {record?.productName?.length > 30
                ? record.productName.slice(0, 30) + "..."
                : record?.productName}
            </span>
